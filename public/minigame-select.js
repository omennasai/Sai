(()=>{
  ...
  function ensureUI(){
    ...
    picker.innerHTML='<div class="mgLabel">미니게임 선택</div><div class="mgChoices"><button type="button" data-game="falling_tiles">떨어지는 발판</button><button type="button" data-game="box_contest">박스 쟁탈전</button><button type="button" data-game="bow_battle">활전</button></div><div id="mgDesc"></div>';
    ...
    style.textContent=`
      ...
      .mgChoices{display:grid;grid-template-columns:repeat(3,1fr);gap:6px}.mgChoices button{margin:0;padding:9px 5px;font-size:11px;background:#1a2230;color:#dbe4f2;border:1px solid #ffffff20;white-space:nowrap}
      ...
      @media(max-width:700px) and (orientation:portrait){.lobbyCard{top:96px!important;bottom:auto!important;right:12px!important;width:230px!important}.mgChoices{grid-template-columns:1fr 1fr}.mgChoices button:last-child{grid-column:1/-1}}
    `;
  ...
  function render(){
    ...
    if(typeof startBtn!=='undefined'&&startBtn){
      ...
      if(isHost){
        ...
        startBtn.textContent=!g.ready?`${g.name} 제작 중`:(enough?'게임 시작':'친구 대기 중');
      }
    }
  }
